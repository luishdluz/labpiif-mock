...
function agregarCeldaCodigo(contenido = null) {
  ...
  const celdaActiva = document.querySelector('.celda-activa');
  ...
  // Código por defecto
  const codigoPorDefecto = contenido || `# Hola mundo\nprint("Hola mundo")`;
  ...
  // ✅ Insertar debajo de la celda activa
  if (celdaActiva) {
    celdaActiva.insertAdjacentElement('afterend', celda);
  } else {
  ...
  require(['vs/editor/editor.main'], function () {
    ...
    window.editorActivo = editor;
    window.resultadoActivo = resultadoDiv;

    // ✅ Manejo de celda activa
    celda.addEventListener('click', () => {
      document.querySelectorAll('.celda').forEach(c => c.classList.remove('celda-activa'));
      celda.classList.add('celda-activa');
...
document.getElementById('btnAgregarCodigo').addEventListener('click', function () {
    agregarCeldaCodigo(null);
});
...
function agregarCeldaTexto() {
  ...
  if (celdaActiva) {
    ...
    // Si no hay celda activa, la agrega al final
    notebook.appendChild(celda);
  }

  // ✅ Activar esta celda automáticamente
  document.querySelectorAll('.celda').forEach(c => c.classList.remove('celda-activa'));
  celda.classList.add('celda-activa');

  // Limpiar editor activo
  window.editorActivo = null;
  window.resultadoActivo = null;

  // Evento para activar al hacer clic
  celda.addEventListener('click', () => {
    document.querySelectorAll('.celda').forEach(c => c.classList.remove('celda-activa'));
    celda.classList.add('celda-activa');
...
function habilitarDragAndDrop() {
  ...
  document.addEventListener('dragover', (e) => {
    e.preventDefault();
    const celdaSobre = e.target.closest('.celda');

    if (!celdaArrastrada || !celdaSobre || celdaArrastrada === celdaSobre) {
      return;
    }

    const notebook = document.getElementById('notebook-scroll');
    const rect = celdaSobre.getBoundingClientRect();
    const mitad = rect.top + rect.height / 2;

    if (e.clientY < mitad) {
      notebook.insertBefore(celdaArrastrada, celdaSobre);
    } else {
      notebook.insertBefore(celdaArrastrada, celdaSobre.nextSibling);
    }
  });
}
