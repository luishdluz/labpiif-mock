...
let celdaActiva = null;
  ...
  // Si es celda de código, conservar editor activo
  if (celda.classList.contains('celda-codigo')) {
    const editorId = celda.querySelector('.editor')?.id;
    if (editorId && window.editores && window.editores[editorId]) {
      window.editorActivo = window.editores[editorId].editor;
      window.resultadoActivo = window.editores[editorId].resultado;
    }
  } else {
    // Si es celda de texto, limpiar editor activo
...
function agregarCeldaCodigo(contenido = null) {
  ...
  // ✅ USAR SOLO ESTA FORMA DE CARGA
  require(['vs/editor/editor.main'], function () {

    const nuevoEditor = monaco.editor.create(editorDiv, {
      value: contenido || `# Código por default\nprint("Hola mundo")`,
      language: 'python',
      theme: 'vs-light',
    ...
    console.error("❌ ERROR AL CARGAR MONACO:", err);
  });
}
